...
public class ClienteController {
   ...
   @PostMapping   public ResponseEntity<Response<ClienteDto>> salvar(@Valid @RequestBody ClienteDto clienteDto, BindingResult result) {
         ...
         try {
                ...
                if (result.hasErrors()) {
                       ...
                       for (int i = 0; i < result.getErrorCount(); i++) {                          
                    	   response.adicionarErro(result.getAllErrors().get(i).getDefaultMessage());                       }                                              log.info("Controller: Os campos obrigatórios não foram preenchidos");                       return ResponseEntity.badRequest().body(response);
